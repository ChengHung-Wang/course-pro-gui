    ...
    <div class="container m-0">
      <div class="row">
        ...
        <div class="col-2 mt-3">
          <el-select size="large" v-model="searchKey" placeholder="Select" class="w-100">
        ...
        <div class="col-10 mt-3">
          ...
          >
            <el-option
              ...
              <span style="float: right; color: var(--el-text-color-secondary); font-size: 13px; margin-right: 6px;">
              {{ item.value }}
            </span>
            </el-option>
          </el-select>
        ...
        <div class="col-12 mt-3">
          <el-button class="w-100" size="large" type="primary" plain @click="pushTarget">加入選課清單</el-button>
        </div>
      </div>
    </div>
      ...
      <div class="row">
        <div class="col-12">
          <el-table :data="courseStore.targets" style="width: 100%">
            <el-table-column prop="course.serial" label="課程序號" width="120"/>
            <el-table-column prop="course.name_zh" label="課程名稱" width="240"/>
            <el-table-column prop="course.teacher.name_zh" label="授課教師" width="150"/>
            <el-table-column prop="course.points" label="學分" width="60"/>
            <el-table-column prop="course.hold_on" label="上課時間" width="120"/>
            <el-table-column prop="status" label="狀態"/>
            <el-table-column prop="tried" label="嘗試次數"/>
            <el-table-column class-name="target-operate" fixed="right" label="操作" width="180">
              <template #default="scope">
                <el-button link type="primary" size="small" :icon="Search" @click="ElMessage('開發中...')">Detail</el-button>
                <el-button link type="danger"  size="small" :icon="Delete" @click="delTarget(scope.row.course_id)">Delete</el-button>
              </template>
            </el-table-column>
          </el-table>
        </div>
...
const remoteMethod = async (query: string) => {
  if (query) {
    ...
    await courseStore.searchCourse("1112", query, toRaw(searchKey).value).then(res => {
      loading.value = false;
      if (res.status != 200) {
        ElMessage({
      ...
      options.value = res.res.data.map((el: any) => {
        return {
          label: `${el.serial} - ${el.name_zh} - ${el.original.CourseTeacher} (${el.hold_on.join(", ")})`,
