...
import type { CourseMyHistoryAvgApi } from "@/models/api/course/my/history/avg";
import type { CourseSearchApi } from "@/models/api/course/search";
...
interface State {
  appLoading: boolean;
  schedule: Pick<
    CourseMyScheduleApi["data"],
    "courses" | "timeline" | "total_credit"
  > & {
    colAmount: number;
    rowAmount: number;
  };
  ...
  targets: CourseMyTargetApi["data"];
  dayIcons: Record<keyof CourseMyScheduleApi["data"]["timeline"], URL>;
  menu: {
    items: {
      displayName: string;
      icon: string;
...
export const useCourseStore = defineStore("course", {
  ...
  actions: {
    ...
    async getScheduleSummary() {
      ...
      this.schedule.total_credit = result.res.data.total_credit;
      return result;
    },
    getScheduleTable() {
      const result: any = [];
    ...
    async getGPA() {
      return await request<CourseMyHistoryAvgApi>(
        "GET",
        "/course/my/history/avg"
      );
    },
    async searchCourse(semester: string, keyword: string, key: string) {
      ...
      params[key] = keyword;
      return await request<CourseSearchApi>("POST", "/course/search", params);
    },
    async getMaximumCredits() {
      ...
      latestGPA >= 3.38
        ? (this.maximumCredits = 31)
        : (this.maximumCredits = 25);
      return this.maximumCredits;
    },
    async getTarget() {
      const result = await request<CourseMyTargetApi>(
        "GET",
        "/course/my/target"
      );
      if (result.status == 200) {
